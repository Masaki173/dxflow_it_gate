...
<header class="py-4">
        <div class="mt-4 bg-white dark:bg-gray-800">
          <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 text-center">
            ...
            <form method="GET" action="{{ route('network.logs') }}">
                <button type="submit"
                        ...
                        <tbody class="divide-y divide-gray-300 bg-white">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2 border-r border-gray-300">
                                    {{ \App\Models\Inquiry::NETWORK_OPTIONS[$item->network_option] ?? '不明' }}
                                </td>
                                ...
                                <td class="px-4 py-2 border-r border-gray-300">
                                    <form method="POST"
                                        action="{{ route('network.markHandled',$item->id) }}">
                                        @csrf
                                        <select name="can_handle" required class="border rounded px-3 pr-16 py-1 h-8 text-sm">
